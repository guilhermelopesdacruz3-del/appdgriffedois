...
const TIPOS: { value: Tipo; label: string; color: string; icon: string }[] = [
  { value: "cupom", label: "Cupom", color: "bg-amber-400/10 border border-amber-300/20 text-amber-200", icon: "🎟️" },
  { value: "promocao", label: "Promoção", color: "bg-rose-400/10 border border-rose-300/20 text-rose-200", icon: "🔥" },
  { value: "produto", label: "Produto exclusivo", color: "bg-violet-400/10 border border-violet-300/20 text-violet-200", icon: "✨" },
  { value: "carrinho", label: "Aviso de carrinho", color: "bg-sky-400/10 border border-sky-300/20 text-sky-200", icon: "🛒" },
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-white/60">Envio custa 1 notificação por cliente</span>
      </div>

      <div className="bg-white/5 border border-white/10 rounded-2xl p-4 space-y-3 backdrop-blur">
        <p className="text-[11px] font-bold text-white/80">Modelos rápidos</p>
        <div className="flex gap-2 overflow-x-auto pb-1">
          {TEMPLATES.map((tpl) => (
            <button
              key={tpl.label}
              onClick={() => { setTitulo(tpl.titulo); setCorpo(tpl.corpo); setTipo(tpl.tipo); }}
              className="shrink-0 px-3 py-2 rounded-xl border border-white/10 bg-white/10 text-[10px] font-semibold text-white hover:border-white/20"
            >
              {tpl.label}
            </button>
          ))}
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-4 shadow-sm space-y-3">
        <div className="flex gap-2">
          {TIPOS.map((item) => (
            <button
              ...
              className={`flex-1 py-2 rounded-xl border text-[10px] font-bold transition-all ${tipo === item.value ? `${item.color} border-current` : "border-white/10 text-gray-300"}`}
            >
        ...
        <div>
          <label className="text-[11px] text-white/70">Título</label>
          <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Ex: Cupom de 10% OFF" className="w-full h-10 px-3 rounded-xl border border-white/10 bg-black/40 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-gold" />
        </div>
        <div>
          <label className="text-[11px] text-white/70">Mensagem</label>
          <textarea value={corpo} onChange={(e) => setCorpo(e.target.value)} placeholder="Ex: Use o cupom SAUDE10..." rows={3} className="w-full px-3 py-2 rounded-xl border border-white/10 bg-black/40 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-gold" />
        </div>

        {/* Pré-visualização mobile */}
        <div className="rounded-2xl border border-white/10 bg-white/5 p-3">
          <p className="text-[10px] font-bold text-white/80 mb-2">Pré-visualização</p>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-4 shadow-sm space-y-3">
        <p className="text-[11px] font-bold text-white/80">Destinatários (filtros)</p>
        <input value={fEmail} onChange={(e) => setFEmail(e.target.value)} placeholder="Filtrar por e-mail (parte)" className="w-full h-10 px-3 rounded-xl border border-white/10 bg-black/40 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-gold" />
        <input value={fNome} onChange={(e) => setFNome(e.target.value)} placeholder="Filtrar por nome" className="w-full h-10 px-3 rounded-xl border border-white/10 bg-black/40 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-gold" />
        <input value={fPontosMin} onChange={(e) => setFPontosMin((e.target.value || "").replace(/\D+/g, "").slice(0, 20))} placeholder="Nível mínimo de fidelidade (pontos)" className="w-full h-10 px-3 rounded-xl border border-white/10 bg-black/40 text-white text-sm placeholder:text-white/30 focus:outline-none focus:border-gold" />
        <p className="text-[11px] text-white/60">{preview.length} cliente(s) correspondem aos filtros.</p>
      </div>

      {status && <p className="text-[11px] text-center text-gold">{status}</p>}

      <button onClick={enviar} disabled={enviando} className="w-full h-12 bg-white text-black text-xs font-bold rounded-2xl disabled:opacity-60">
        {enviando ? "Enviando..." : "Enviar notificação"}
      </button>

      {historico.length > 0 && (
        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 shadow-sm space-y-2">
          <p className="text-[11px] font-bold text-white/80">Histórico recente</p>
