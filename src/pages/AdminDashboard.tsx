...
import { relatorioAdmin, listarPedidosAdmin, listarSituacoes, buscarClienteAdmin, listarLogsAdmin } from "../services/admin";
...
export default function AdminDashboard({ token }: { token: string }) {
  ...
  const [aba, setAba] = useState<"visao" | "cupons" | "logs">("visao");

  useEffect(() => {
    const carregar = async () => {
      setLoading(true);
      try {
        const [rel, ped, sit, cli, log, cup] = await Promise.all([
          relatorioAdmin(),
          listarPedidosAdmin({ limit: 50 }),
          listarSituacoes(),
          buscarClienteAdmin("").then(() => []).catch(() => []),
        ...
        setSituacoes(sit || []);
        setClientes((cli as any).clientes || []);
        setLogs(log.logs || []);
        setCupons(cup || []);
      } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 px-4 pt-3">
        {(["visao", "cupons", "logs"] as const).map((a) => (
          <button
            key={a}
            onClick={() => setAba(a)}
          ...
          >
            {a === "visao" ? "Visão Geral" : a === "cupons" ? "Cupons" : "Logs"}
          </button>
        ))}
      </div>
              ...
              {clientes.slice(0, 20).map((c, i) => (
                <div key={c.email} className="flex items-center justify-between text-[11px]">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-gray-400">{c.pedidos} pedidos</p>
                  </div>
                </div>
              ))}
            </div>
